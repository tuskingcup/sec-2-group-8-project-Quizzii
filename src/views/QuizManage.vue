...
const removeQuiz = async (removeNoteid)=>{
  const res = await fetch(`http://localhost:5000/${params.title}/${removeNoteid}`,{
    method:'DELETE'
  })
  if(res.status === 200){
...
const modifyMode = async (edit) => {
    ...
    method:'PUT',
    headers:{'content-type':'application/json'},
    body: JSON.stringify({  question : edit.question ,
                            A: edit.A,
                            B: edit.B,
                            C: edit.C,
                            answer: edit.answer })
  ...
  if(res.status === 200){
    ...
    quizList.value = quizList.value.map((note)=> note.id === modifyNote.id ? {...note,  question : modifyNote.question , a: modifyNote.a, b: modifyNote.b, c: modifyNote.c, answer: modifyNote.answer}: note)
...
<template>
<div>
    <button @click="goBack">Back</button> <br><br>
    <div>
        <button @click="showForm" :disabled="onForm" >create Quiz</button> <br>
    <span v-show="onForm">
        <Form :currentQuiz="editingNote" @editQuiz="modifyMode" @createQuiz="createQuiz" @closeForm="showForm" />
    </span>
 </div><br>
