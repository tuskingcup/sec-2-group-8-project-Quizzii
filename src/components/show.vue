    ...
    <div v-for="(quiz, index) in quizList" class="absoluted my-8 mx-16">
      <div class="px-10 py-5 bg-base-200 rounded-lg hover:bg-base-300 hover:drop-shadow-[0_15px_15px_rgba(255,255,255,.1)]">
        <div class="">
          ...
          <button class="bg-amber-400 px-5 rounded-xl hover:scale-110" @click="$emit('editQuiz', quiz)">Edit</button> &nbsp;
          <button class="border px-5 rounded-xl text-white hover:scale-110 hover:bg-base-200" v-show="quiz.id >= 4" @click="$emit('deleteQuiz', quiz.id, quiz.question)">
            Delete
          </button>
          </div>
        </div>
